...
OpenPedCan analyses include open-access data associated with the seven diseases present in the TARGET dataset: Acute Lymphoblastic Leukemia (ALL), Acute Myeloid Leukemia (AML), Clear cell sarcoma of the kidney, Neuroblastoma, Osteosarcoma, Rhabdoid tumor, and Wilm’s Tumor.
...
OpenPedCan project includes 10,414 open-access TCGA RNA-Seq samples (716 normal and 9698 tumor) from [33 cancer types](https://github.com/rokitalab/OpenPedCan-Project-CNH/blob/dev/analyses/molecular-subtyping-integrate/results/tcga_cancer_groups.tsv).
...
The inclusion of GTEx and adult TCGA data specifically allows for tumor-specific gene expression profiling. 
...
## Re-use potential
OpenPedCan represents a valuable resource, not only by significantly extending OpenPBTA to include more than 5,000 additional patients and 6,000 tumors, but also my adding a number of new data types not previously included, such as methylation, miRNA, and adult RNASeq data. 
...
By providing this data in a harmonized manner, we enable investigators to reduce the financial and time-related costs associated with their analyses, which would otherwise total hundreds of project hours and over $50,000 in data analysis alone [@doi: 10.1038/s41597-019-0096-4].
