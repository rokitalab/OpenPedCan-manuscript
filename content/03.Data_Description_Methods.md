...
In September of 2018, the [Children's Brain Tumor Network (CBTN)](https://cbtn.org/) [@doi:10.1016/j.neo.2022.100846] released the Pediatric Brain Tumor Atlas (PBTA), a genomic dataset (whole genome sequencing, whole exome sequencing, RNA sequencing, proteomic, and clinical data) for nearly 1,000 tumors, available from the Gabriella Miller Kids First Portal [@url:https://kidsfirstdrc.org].
...
First, we sought to harmonize, summarize, and contextualize pediatric cancer genomics data among normal tissues (GTEx) and adult cancer tissues (TCGA) to enable the creation of the National Cancer Institute's Molecular Targets Platform (MTP) [@url:https://moleculartargets.ccdi.cancer.gov].
...
Instead of the b-allele frequency germline input file, we used the `dbSNP_v153_ucsc-compatible.converted.vt.decomp.norm.common_snps.vcf.gz` dbSNP common snps file at https://kids-first-seq-data.s3.amazonaws.com/pipeline_references/dbSNP_v153_ucsc-compatible.converted.vt.decomp.norm.common_snps.vcf.gz and to avoid hard-to-call regions, utilized the `hg38_canonical_150.mappability` mappability file athttps://s3.amazonaws.com/kids-first-seq-data/pipeline_references/hg38_canonical_150.mappability.
Both are also linked in the public Kids First references CAVATICA project at https://cavatica.sbgenomics.com/u/kfdrc-harmonization/kf-references.
The Control-FREEC tumor only workflow can be found in the Kids First GitHub repository at https://github.com/kids-first/kf-tumor-workflow/blob/v0.3.0-beta/workflows/kfdrc_controlfreec_tumor_only_wf.cwl.
...
The required data files are also available publicly on CAVATICA at [https://cavatica.sbgenomics.com/u/cavatica/opentarget/files/#q?path=v15](https://cavatica.sbgenomics.com/u/cavatica/opentarget/files/#q?path=v15).
...
The liftover process was done via a custom script at [https://github.com/d3b-center/D3b-DGD-Collaboration/blob/main/scripts/update_gene_symbols.py](https://github.com/d3b-center/D3b-DGD-Collaboration/blob/main/scripts/update_gene_symbols.py). 
The script first constructs an object detailing the gene symbol changes from the HGNC symbol database at [https://ftp.ebi.ac.uk/pub/databases/genenames/hgnc/archive/monthly/tsv/hgnc_complete_set_2021-06-01.txt](https://ftp.ebi.ac.uk/pub/databases/genenames/hgnc/archive/monthly/tsv/hgnc_complete_set_2021-06-01.txt). 
